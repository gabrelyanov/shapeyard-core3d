...
enum class Operation:std::uint8_t {CreateEnclosure=1,RebuildEnclosure=2,CreateAssembly=3,RebuildProfile=4,CreateProfile=5};
enum class Recipe:std::uint8_t {Profile=1,Enclosure=2};
struct Part {Recipe recipe=Recipe::Profile;std::uint32_t schema=0;std::string name;std::vector<double> values;};
struct Descriptor {Operation operation=Operation::CreateProfile;std::vector<Part> parts;};
struct Key {
...
inline bool Encode(const Descriptor&d,std::vector<std::uint8_t>&out)noexcept{
    out.clear();try{
        if(d.operation<Operation::CreateEnclosure||d.operation>Operation::CreateProfile||d.parts.empty()||d.parts.size()>MaximumParts
            ||(d.operation!=Operation::CreateAssembly&&d.parts.size()!=1))return false;
        Writer w;w.raw(reinterpret_cast<const std::uint8_t*>("SYMD"),4);w.integer(1,1);w.integer(std::uint8_t(d.operation),1);w.integer(d.parts.size(),1);w.integer(0,1);
        for(const auto&p:d.parts){const auto expected=(d.operation==Operation::CreateEnclosure||d.operation==Operation::RebuildEnclosure)?Recipe::Enclosure:Recipe::Profile;
            if(p.recipe!=expected||p.schema<1||p.schema>(p.recipe==Recipe::Profile?4u:2u)||!UTF8(p.name,256,true)||p.values.empty()||p.values.size()>MaximumValues
...
inline bool Decode(const std::vector<std::uint8_t>&bytes,Descriptor&out)noexcept{
    out={};try{
        if(bytes.size()<8||bytes.size()>MaximumBytes||std::memcmp(bytes.data(),"SYMD",4)||bytes[4]!=1||bytes[7]!=0)return false;
        ...
        Descriptor d;d.operation=Operation(bytes[5]);const auto count=bytes[6];if(!count||count>MaximumParts)return false;
