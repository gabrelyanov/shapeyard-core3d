...
namespace core3d {
    ...
    using MeshRegionInsetSnapshot = MeshRegionExtrudeSnapshot;
    ...
    std::optional<StoredRectangularLoftSnapshot> storedRectangularLoftDefinition(
        ...
        std::optional<MeshRegionInsetSnapshot> prepareMeshRegionInset(
            const ObjectFrameIdentity& identity,std::uint64_t presentationRevision,
            std::uint32_t width,std::uint32_t height,std::uint32_t seedTriangle) noexcept;
        OrdinaryEditResult commitMeshRegionInset(const std::string& sessionIdentifier,
            double distanceMM) noexcept;
        void cancelMeshRegionInset(const std::string& sessionIdentifier) noexcept;
    ...
    private:
        ...
        std::shared_ptr<MeshRegionInsetWork> _meshRegionInsetWork;
