//
//  Core3DManipulator.hpp
//  Core3D
//
//  Created by Dmitry Sukhorukov on 04.06.2024.
//

#ifndef Core3DManipulator_hpp
#define Core3DManipulator_hpp
...
public:
  ...
  Standard_EXPORT virtual void SetPart (const Standard_Integer theAxisIndex, const AIS_ManipulatorMode theMode, const Standard_Boolean theIsEnabled);
  ...
  //! It is placed in the center of object bounding box, and its size is adjusted to the object bounding box.
  Standard_EXPORT void Attach (const Handle(Core3DManipulatorObjectSequence)& theObject, const OptionsForAttach& theOptions = OptionsForAttach());
...
public:
  ...
  Standard_EXPORT void StartTransform (const Standard_Integer theX, const Standard_Integer theY, const Handle(Core3DView)& theView, const Handle(AIS_InteractiveContext)& theCtx);
  ...
  Standard_EXPORT void NonUniformScale (const Standard_Real scaleFactor, const Handle(AIS_InteractiveContext)& theCtx);
  Standard_EXPORT void UpdateCachedShapes();
  ...
  Standard_EXPORT void Detach (const Handle(AIS_InteractiveObject)& theObject);

  //! @return all owning objects.
  Standard_EXPORT Handle(Core3DManipulatorObjectSequence) Objects() const;
...
public: //! @name Setters for parameters
  ...
  AIS_ManipulatorMode LastTapped () { return AIS_MM_MirroringPlaneNeg; }
...
public:
  ...
  Handle(TopLoc_Datum3D) Transform() {
...
public: //! @name Presentation computation
  ...
  Standard_EXPORT virtual void HilightOwnerWithColor (const Handle(PrsMgr_PresentationManager)& thePM,
	...
	const std::map<Handle(AIS_InteractiveObject), TopoDS_Shape>& cachedShapes() const { return mySourceShapes; }
...
protected:
  ...
  Standard_EXPORT void attachToBox (const Bnd_Box& theBox, const gp_Ax2& anAx2);
...
protected: //! @name Auxiliary classes to fill presentation with proper primitives
  ...
  class Plane : public Triangles
  {
  public:
	  Plane() { }
	  ~Plane() { }
	  
	  void Init (const gp_Ax1& thePosition, const Standard_ShortReal theSize, const Standard_ShortReal planeSize);
  ...
  public:
    ...
    void Compute (const Handle(PrsMgr_PresentationManager)& thePrsMgr,
				  ...
				  const Standard_Boolean biDirected = false);
    ...
    void SetTransformPersistence (const Handle(Graphic3d_TransformPers)& theTrsfPers)
    {
	  ...
	  {
		  myHighlightScalerUniform->SetTransformPersistence (theTrsfPers);
	  }
	  ...
	  {
		  myHighlightMirroringPos->SetTransformPersistence(theTrsfPers);
	  }
	  if (!myHighlightMirroringNeg.IsNull())
	  {
		  myHighlightMirroringNeg->SetTransformPersistence(theTrsfPers);
	  }
    }
    ...
    void Transform (const Handle(TopLoc_Datum3D)& theTransformation)
    {
	  ...
	  if (!myHighlightScalerUniform.IsNull())
	  {
		  myHighlightScalerUniform->SetTransformation (theTransformation);
	  }
	  ...
	  if (!myHighlightMirroringPos.IsNull())
	  {
		  myHighlightMirroringPos->SetTransformation(theTransformation);
	  }
		
	  if (!myHighlightMirroringNeg.IsNull())
	  {
		  myHighlightMirroringNeg->SetTransformation(theTransformation);
	  }
    }
	...
	Standard_Boolean HasScalingUniform() const { return myHasScalingUniform; }

    Standard_Boolean HasDragging() const { return myHasDragging; }
	  
	Standard_Boolean HasMirroringPos() const { return myHasMirroringPos; }
	  
	Standard_Boolean HasMirroringNeg() const { return myHasMirroringNeg; }
	...
	void SetScalingUniform (const Standard_Boolean theIsEnabled) { myHasScalingUniform = theIsEnabled; }

    void SetDragging(const Standard_Boolean theIsEnabled) { myHasDragging = theIsEnabled; }
	  
    void SetMirroringPos(const Standard_Boolean theIsEnabled) { myHasMirroringPos = theIsEnabled; }
	  
	void SetMirroringNeg(const Standard_Boolean theIsEnabled) { myHasMirroringNeg = theIsEnabled; }
	...
	const Handle(Prs3d_Presentation)& ScalerHighlightUniformPrs() const { return myHighlightScalerUniform; }

    const Handle(Prs3d_Presentation)& DraggerHighlightPrs() const { return myHighlightDragger; }
	  
	const Handle(Prs3d_Presentation)& MirroringHighlightPosPrs() const { return myHighlightMirroringPos; }
	  
	const Handle(Prs3d_Presentation)& MirroringHighlightNegPrs() const { return myHighlightMirroringNeg; }
	...
	const Handle(Graphic3d_Group)& ScalerUniformGroup() const { return myScalerUniformGroup; }

    const Handle(Graphic3d_Group)& DraggerGroup() const { return myDraggerGroup; }
	  
	const Handle(Graphic3d_Group)& MirroringGroupPos() const { return myMirroringGroupPos; }
	  
	const Handle(Graphic3d_Group)& MirroringGroupNeg() const { return myMirroringGroupNeg; }

    const Handle(Graphic3d_ArrayOfTriangles)& TriangleArrayF() const { return myTriangleArrayF; }
	const Handle(Graphic3d_ArrayOfTriangles)& TriangleArrayB() const { return myTriangleArrayB; }
  ...
  public:
	...
	const Sphere& ScalerSphereUniform() const { return mySphereUniform; }
	const gp_Pnt& ScalerSphereUniformPosition() const { return mySphereUniformPos; }
	const Plane& MirroringPlanePos() const { return myPlanePos; }
	const gp_Pnt& MirroringPlanePosPosition() const { return myPlanePosPos; }
    const Plane& MirroringPlaneNeg() const { return myPlaneNeg; }
    const gp_Pnt& MirroringPlaneNegPosition() const { return myPlanePosNeg; }
  ...
  protected:
	...
	Standard_Boolean myHasScalingUniform;
    Standard_ShortReal myBoxSize; //!< Size of scaling cube.

	Standard_Boolean myHasMirroringPos;
	Standard_Boolean myHasMirroringNeg;
	Standard_ShortReal myMirroringPlaneSize;
  ...
  protected:
    ...
    Cube     myCube;
    gp_Pnt   myCubePos;
	Sphere     mySphereUniform;
	gp_Pnt   mySphereUniformPos;
	Plane 	 myPlanePos;
	Plane 	 myPlaneNeg;
	gp_Pnt   myPlanePosPos;
	gp_Pnt   myPlanePosNeg;

    Handle(Graphic3d_Group) myTranslatorGroup;
    Handle(Graphic3d_Group) myScalerGroup;
	Handle(Graphic3d_Group) myScalerUniformGroup;
    Handle(Graphic3d_Group) myRotatorGroup;
    Handle(Graphic3d_Group) myDraggerGroup;
	Handle(Graphic3d_Group) myMirroringGroupPos;
	Handle(Graphic3d_Group) myMirroringGroupNeg;

    Handle(Prs3d_Presentation) myHighlightTranslator;
    Handle(Prs3d_Presentation) myHighlightScaler;
	Handle(Prs3d_Presentation) myHighlightScalerUniform;
    Handle(Prs3d_Presentation) myHighlightRotator;
    Handle(Prs3d_Presentation) myHighlightDragger;
	Handle(Prs3d_Presentation) myHighlightMirroringPos;
	Handle(Prs3d_Presentation) myHighlightMirroringNeg;

    Handle(Graphic3d_ArrayOfTriangles) myTriangleArrayF;
	Handle(Graphic3d_ArrayOfTriangles) myTriangleArrayB;
...
protected:
  ...
  BehaviorOnTransform myBehaviorOnTransform; //!< Behavior settings applied on manipulator when transforming an object.
	//!
  Standard_Real myOldScaleFactor;
  Standard_Integer myOldIndexScale;
  std::map<Handle(AIS_InteractiveObject), TopoDS_Shape> mySourceShapes;
...
//  Handle(Core3DManipulatorObjectSequence) myObjectSeq;
	
public:

  DEFINE_STANDARD_RTTIEXT(Core3DManipulator, AIS_InteractiveObject)
};

#endif /* Core3DManipulator_hpp */
